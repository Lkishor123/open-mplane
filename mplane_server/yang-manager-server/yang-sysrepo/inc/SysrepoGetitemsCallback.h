...
private:
 ...
 // Given a node name of the form: "o-ran-hardware:product-code" returns
 // "o-ran-hardware". If name has no ':' then returns empty string
 static std::string nodeModule(const std::string& name);

 // Given a node name of the form: "o-ran-hardware:product-code" returns
 // "product-code". If name has no ':' then returns name string
 static std::string nodeName(const std::string& name);

private:
 std::string mParentPath; //!> XPath to the parent node used while creating tree
