...
//-------------------------------------------------------------------------------------------------------------
std::string
SysrepoGetitemsCallback::path() const {
  return mParentPath;
}

//-------------------------------------------------------------------------------------------------------------
int
SysrepoGetitemsCallback::oper_get_items(
  ...
  try {
    ...
    std::cerr << "[error] libyang exception: oper_get_items() - "
              << mParentPath.c_str() << std::endl;
  ...
  for (auto element : elementContainer->contents()) {
    try {
      ...
      if (element->isContainer()) {
        DISP_ELES(
            std::cerr << levelStr << "is container " << element->name() << ":"
                      << std::endl);
      ...
      std::string exceptionStr = exception.what();
      std::cerr << "[error] libyang exception: processElements() - "
                << mParentPath.c_str() << std::endl;
      std::cerr << "[error] " << exceptionStr << std::endl;
    }
  }
...
//-------------------------------------------------------------------------------------------------------------
std::string
SysrepoGetitemsCallback::nodeModule(const std::string& name) {
...
//-------------------------------------------------------------------------------------------------------------
std::string
SysrepoGetitemsCallback::nodeName(const std::string& name) {
  std::size_t pos(name.find(":"));
  if (pos == std::string::npos)
    return name;
