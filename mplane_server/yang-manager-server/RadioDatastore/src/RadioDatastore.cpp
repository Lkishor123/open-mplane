...
bool
RadDsEvtMgr::add(RadDsEvt_t event) {
  mEvents.push(event);
  return true;
}

bool
RadDsEvtMgr::getNext(RadDsEvt_t& event) {
  if (mEvents.empty())
    return false;
...
IRadioDataStore::State_E
RadioDatastore::getState(void) const {
  return mState;
}

bool
RadioDatastore::isState(IRadioDataStore::State_E state) const {
  return (mState == state);
}

YangResult_E
RadioDatastore::activate(void) {
  return (setState(State_E::ACTIVE) ? YangResult_E::OK : YangResult_E::FAIL);
}

bool
RadioDatastore::isModuleInstalled(const std::string& name) const {
  bool installed = false;

  if (std::find(mModsInstalled.begin(), mModsInstalled.end(), name) !=
      mModsInstalled.end()) {
    installed = true;
  }
...
RadioDatastore::getListOfInstalledModules(void) const {
 ...
 */
bool
RadioDatastore::getNextEvent(RadDsEvt_t& event) {
  return mEvtMgr->getNext(event);
}

YangResult_E
RadioDatastore::dumpInstalledModules(void) {
...
bool
RadioDatastore::setState(IRadioDataStore::State_E state) {
...
bool
RadioDatastore::addInstalledModule(const std::string& name) {
