...
int halmplane_configPerfMeasurementParams(struct performance_measurement_params_t* config) {
  (void)config; return 0;
}

int halmplane_activateTransceiverMeasObjects(struct transceiver_measurement_objects_t config,
                                             halmplane_transceiver_meas_cb_t cb) {
  (void)config; g_txrx_cb = cb; return 0;
}

int halmplane_activateRxWindowMeasObjects(struct rx_window_measurement_objects_t config,
                                          halmplane_rx_window_meas_cb_t cb) {
  (void)config; g_rxwin_cb = cb; return 0;
}

int halmplane_activateTxMeasObjects(const struct tx_measurement_objects_t config,
                                    halmplane_tx_stats_meas_cb_t cb) {
  (void)config; g_tx_cb = cb; return 0;
}

int halmplane_activateEpeMeasObjects(struct epe_measurement_objects_t config,
